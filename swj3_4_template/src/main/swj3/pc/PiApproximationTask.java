...
public record PiApproximationTask(int terms, int precision) implements Task {
  ...
  public void execute() {
    try {
      ...
      System.out.println("%s (%s)%n  pi ≈ %s".formatted(this, Thread.currentThread().getName(), pi));
    } catch (InterruptedException e) {
      System.out.println("%s (%s)%n  interrupted".formatted(this, Thread.currentThread().getName()));
